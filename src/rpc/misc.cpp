...
bool getAddressesFromParams(const Config &config, const UniValue& params, std::vector<std::pair<uint160, int> > &addresses) {
    if (params[0].isStr()) {
        std::string address = params[0].get_str();
        uint160 hashBytes;
        int type = 0;
        if (!GetDestinationIndexKey(config.GetChainParams(), address, hashBytes, type)) {
...
UniValue getaddressmempool(const Config &config, const JSONRPCRequest& request)
{
    ...
    if (!getAddressesFromParams(config, request.params, addresses)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }
